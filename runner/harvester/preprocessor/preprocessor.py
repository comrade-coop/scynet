...
class Preprocessor(Signal):
    def __init__(self, signal, preprocess_window_length=1, output_window_length=-1, windowing_strategy='partial', *args, **kwargs):
        ...
        if self.output_window_length > 0:
            self.shape = (output_window_length,) + self.shape
        if self.windowing_strategy == 'previous':
            self.available_from = self.available_from + self.preprocess_window_length * self.granularity

    def iterate(self, from_date, to_date):
        window = deque(maxlen=self.preprocess_window_length)

        if self.windowing_strategy == 'previous':
            for row in self.signal.iterate(from_date - self.preprocess_window_length * self.granularity, from_date):
                window.append(row[1])

        for row in self.signal.iterate(from_date, to_date):
            window.append(row[1])
            if self.windowing_strategy == 'first' or self.windowing_strategy == 'previous':
                if len(window) == self.preprocess_window_length:
