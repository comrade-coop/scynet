...
object messages {
  case class Block(hash: String, number: BigInteger, timeStamp: BigInteger)
  case class Transaction(block: TransactionBlock, receipt: Receipt, chainId: Long, creates: Option[String], from: String, gas: BigInteger, gasPrice: BigInteger,
                         hash: String, input: String, nonce: BigInteger, publicKey: String, to: String, index: BigInteger, indexRaw: String, value: BigInteger, srv: SRV /* , traces: List[Trace] */)
