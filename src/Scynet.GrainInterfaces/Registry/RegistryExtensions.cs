...
namespace Scynet.GrainInterfaces.Registry
{
    ...
    public static class RegistryExtensions
    {
        ...
        /// <param name="timeout">The time after which the subscription should be unsubscribed automatically</param>
        public static Task Subscribe<K, T>(this IRegistry<K, T> registry, Expression<Func<K, T, bool>> query, IRegistryListener<K, T> listener, String @ref = "", TimeSpan? timeout = null)
        {
            var expressionNode = query.ToExpressionNode();
            return registry.Subscribe(expressionNode, listener, @ref, timeout);
        }
    }
}
