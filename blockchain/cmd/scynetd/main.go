...
func main() {
	...
	rootCmd := &cobra.Command{
		Use:               "scynetd",
		Short:             "Scynet App Daemon (server)",
	...
	executor := cli.PrepareBaseCmd(rootCmd, "Scynet", defaultNodeHome)
...
func InitCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			fmt.Printf("Initialized scynetd configuration and bootstrapping files in %s...\n", viper.GetString(cli.HomeFlag))
			return nil
		},
	}
...
$ scynetd add-genesis-account cosmos1tse7r2fadvlrrgau3pa0ss7cqh55wrv6y9alwh 1000STAKE,1000mycoin
`),
